{
  ...
  TFile * f = TFile::Open("GAhh.root");
  TFile * f_red = TFile::Open("GAhh_red.root","RECREATE");
  TH1D * h_gamma = (TH1D *) f->Get("h1D_gamma_GAhh");
  TH1D * h_red_gamma = new TH1D("h_red_gamma","h_red_gamma",900,0,180);

  TH1D * h_deltaB = (TH1D *) f->Get("h1D_delta_B_DK_GAhh");
  TH1D * h_red_deltaB = new TH1D("h_red_delta_B_DK","h_red_delteB_DK",900,0,180);

  TH1D * h_deltaBPi = (TH1D *) f->Get("h1D_delta_B_DPi_GAhh");
  TH1D * h_red_deltaBPi = new TH1D("h_red_delta_B_DPi","h_red_delteB_DPi",900,0,180);

  TH1D * h_delta_B_DKstar0 = (TH1D *) f->Get("h1D_delta_B_DKstar0_GAhh");
  ...
  for (int i=1; i<h_deltaB->GetNbinsX()+1; i++) {
  ...
  for (int i=1; i<h_deltaBPi->GetNbinsX()+1; i++) {
